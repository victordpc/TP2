...
/**
 * A simple MinMax implementation. Adapted from code by Simon Pickin
 */
public class MinMax implements AiAlgorithm {
	...
	public <S extends GameState<S, A>, A extends GameAction<S, A>> A chooseAction(int playerNumber, S state) {
		try {
			Node<S, A> best = minmax(depth, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, playerNumber, state);
			return best.move;
	...
	public <S extends GameState<S, A>, A extends GameAction<S, A>> double evaluateAction(A action, int playerNumber,
			S state) {
		try {
			int d = depth - 1;
			...
			if (state.isFinished() || d < 1) {
				...
				Node<S, A> best = minmax(d, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, playerNumber, state);
				return best.value;
			}
		} catch (InterruptedException ie) {
	...
	private <S extends GameState<S, A>, A extends GameAction<S, A>> Node<S, A> max(int d, double alpha, double beta,
			int player, S state, List<A> actions) throws InterruptedException {
	...
	private <S extends GameState<S, A>, A extends GameAction<S, A>> Node<S, A> min(int d, double alpha, double beta,
			int player, S state, List<A> actions) throws InterruptedException {
	...
	private <S extends GameState<S, A>, A extends GameAction<S, A>> Node<S, A> minmax(int d, double alpha, double beta,
			int playerNumber, S state) throws InterruptedException {
...
//		/**
//		 * Will be called at each leaf evaluation. Should be implemented in a
//		 * thread-safe manner if a multithreaded min-max is going to be used.
//		 */
//		void increment();
//	}
//
//	public static class Node<S extends GameState<S, A>, A extends GameAction<S, A>> implements Comparable<Node<S, A>> {
//		private A move;
...
//				throw new IllegalStateException("applying " + a + " to \n" + state + " actions are " + actions, ise);
//			}
//		}
//
//		return new Node<>(chosen, alpha);
