...
//
//import static es.ucm.fdi.tp.chess.ChessBoard.DIM;
//import static es.ucm.fdi.tp.chess.ChessBoard.empty;
//import static es.ucm.fdi.tp.chess.ChessBoard.enemy;
//import static es.ucm.fdi.tp.chess.ChessBoard.sameTurn;
//
...
//		if (piece == Piece.Pawn) {
//			return pawnCanCapture(p, row, col, dstRow, dstCol, ChessBoard.black(p) ? 1 : -1);
//		} else if (piece.getDeltas() != null) {
//			for (int[] d : piece.getDeltas()) {
//				if (canDeltaCapture(b, p, row, col, d[0], d[1], dstRow, dstCol, piece.hasLinearMotion())) {
//					return true;
//				}
//			}
//		}
...
//	}
//
//	protected static boolean canDeltaCapture(ChessBoard b, byte p, int row, int col, int dy, int dx, int dstRow,
//			int dstCol, boolean multiple) {
//		int dRow = row, dCol = col;
//		do {
//			dRow += dy;
//			dCol += dx;
//			if (dRow == dstRow && dCol == dstCol)
//				return true;
...
//	 *            to search in
//	 * @param turn
//	 *            of king to search for
//	 * @param kingPos
//	 *            to write position into
...
//	}
//
//	protected static boolean pawnCanCapture(byte p, int srcRow, int srcCol, int dstRow, int dstCol, int dy) {
//		return srcRow + dy == dstRow && ((srcCol == dstCol + 1) || (srcCol == dstCol - 1));
//	}
//
//	/**
//	 * @param b
//	 *            board to check in
//	 * @param turn
//	 *            of player that threatens
//	 * @param row
//	 *            of victim
//	 * @param col
//	 *            of victim
//	 * @return true if there is at least 1 piece of turn that can capture the
//	 *         victim
//	 */
...
//
//	private final int canCastle[]; // {white, black}, 0=no, 1=short, 2=long,
//									// 3=both
//
//	private final int enPassant; // -1 if none, column of pawn that advanced
//									// twice otherwise
//
...
//		canCastle = new int[] { CASTLE_SHORT | CASTLE_LONG, CASTLE_SHORT | CASTLE_LONG };
//		enPassant = -1;
//		inCheck = false;
//		valid = null;
//		updateValid();
...
//	 * 
//	 * @param previous
//	 *            board (used mostly to look at turn)
//	 * @param board
//	 *            to use
//	 * @param canCastle
//	 *            representing the state of castling; canCastle[WHITE] is 0 if
//	 *            not allowed, 1 if only short castling still possible, 2 if
//	 *            only long castling still possible 3 if both types of castling
//	 *            are still possible
//	 * @param enPassant
//	 *            with column where enemy pawn just finished a double-advance
//	 * @param inCheck
//	 *            if king is currently in check
//	 */
//	public ChessState(ChessState previous, ChessBoard board, int[] canCastle, int enPassant, boolean inCheck) {
//		super(2);
//		this.board = board;
...
//	 * @param row
//	 * @param col
//	 * @return piece at the given position. Note that chess piece codes are NOT
//	 *         player codes (there are many piece types). See ChessBoard to make
//	 *         sense of the returned int: ChessBoard.black(p) and
//	 *         ChessBoard.white(p) will tell you piece-color
...
//	}
//
//	protected void castlingActions(byte p, int turn, int row, int col, ArrayList<ChessAction> as, int kingRow) {
//		int other = turn == BLACK ? WHITE : BLACK;
//
//		// not in inCheck, empty&non-threatened king transit, empty rook-transit
//		if (!inCheck) {
//			if ((canCastle[turn] & CASTLE_SHORT) != 0 && empty(board.get(kingRow, 5))
//					&& !threatenedBy(board, other, kingRow, 5) && empty(board.get(kingRow, 6))
//					&& !threatenedBy(board, other, kingRow, 6)) {
//				as.add(new ChessAction(turn, row, col, kingRow, 6, Special.ShortCastle));
//			}
//			if ((canCastle[turn] & CASTLE_LONG) != 0 && empty(board.get(kingRow, 1)) && empty(board.get(kingRow, 2))
//					&& !threatenedBy(board, other, kingRow, 2) && empty(board.get(kingRow, 3))
//					&& !threatenedBy(board, other, kingRow, 3)) {
//				as.add(new ChessAction(turn, row, col, kingRow, 2, Special.LongCastle));
//			}
//		}
//	}
//
//	/**
//	 * Used to generate non-pawn actions. Simply looks for empty-or-enemy
//	 * squares along a given direction, given by row and column deltas. If
//	 * 'multiple' is se to true, continues looking until non-empty square found
//	 * (useful for rook, bishop, queen)
//	 */
//	protected void deltaActions(byte p, int turn, int row, int col, int dy, int dx, ArrayList<ChessAction> as,
//			boolean multiple) {
//		int dRow = row, dCol = col;
//		byte target;
//		do {
...
//
//		/*
//		 * static evaluation of positions in chess is a huge topic; feel free to
//		 * improve on this heuristic.
//		 */
//
...
//					score += .01 * Piece.valueOf(p).getValue(!sameTurn(p, turn));
//				}
//			}
//		}
//
...
//		score += canCastle[turn] == 0 ? 0 : (canCastle[turn] == CASTLE_DONE ? .001 : .0005);
//
//		return score;
//	}
//
//	protected void generateActions(byte p, int turn, int row, int col, ArrayList<ChessAction> as) {
//		Piece piece = Piece.valueOf(p);
//		if (piece == Piece.Pawn) {
...
//		} else if (piece.getDeltas() != null) {
//			for (int[] d : piece.getDeltas()) {
//				deltaActions(p, turn, row, col, d[0], d[1], as, piece.hasLinearMotion());
//			}
//			if (piece == Piece.King) {
//				if (ChessBoard.black(p)) {
...
//			if (enPassant == col + 1) {
//				as.add(new ChessAction(turn, row, col, row + dy, col + 1, Special.EnPassant));
...
//		return board.toString() + ("whiteCastle: " + canCastle[0] + " ") + ("blackCastle: " + canCastle[1] + " ")
//				+ (enPassant != -1 ? "passant" + enPassant + " " : "") + (turn == WHITE ? "; white" : "black")
//				+ " to play\n";
//	}
//
//	/**
//	 * Generates valid moves for current player. This can only be called once
//	 * per state (as states are immutable, and results get cached).
//	 */
